...
public class OrderService : IOrderService
{
    ...
    public async Task<ServiceResult<Order>> CreateOrderAsync(int customerId, IReadOnlyList<NewOrderLine> lines)
    {
        ...
        var lineErrors = ValidateLines(lines);
        ...
        var (items, itemErrors) = await BuildOrderItemsAsync(lines);
        if (itemErrors.Count > 0)
            return ServiceResult<Order>.Fail(itemErrors);
    ...
    private static IReadOnlyList<string> ValidateLines(IReadOnlyList<NewOrderLine> lines)
    {
        if (lines is null || lines.Count == 0)
            return new[] { "訂單至少需要一項商品" };

        if (lines.Any(l => l.Quantity <= 0))
            return new[] { "商品數量必須大於 0" };

        if (lines.Select(l => l.ProductId).Distinct().Count() != lines.Count)
            return new[] { "同一商品請勿重複加入，請調整數量即可" };

        return Array.Empty<string>();
    }

    private async Task<(List<OrderItem> Items, List<string> Errors)> BuildOrderItemsAsync(IReadOnlyList<NewOrderLine> lines)
    {
